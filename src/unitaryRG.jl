...
function Renormalisation(
        operatorDef::Dict{String, Function},
        numLegs::Dict{String, Int},
        occupied::Vector{Int64},
        vacant::Vector{Int64},
        allSites::Vector{Int64},
        spins::Dict{Int, Int};
        allowed::Vector=[],
    )
    onShell = vcat(occupied, vacant)
    println(allSites, spins)
    renormalisation = Dict()
    for (name1, def) in operatorDef
        legs = numLegs[name1]
        for right in Iterators.product([allSites for _ in 1:legs]...)
            if intersect(right, occupied) |> isempty && intersect(right, vacant) |> isempty
                continue
            end
            operator, right, factorRight = def(right, spins)
            if factorRight == 0
                continue
            end
            ...
            for (legs, (name2, def)) in zip(numLegs, operatorDef)
                legs = numLegs[name2]
                if !isempty(allowed) && (name1, name2) ∉ allowed
                    continue
                end
                for left in Iterators.product([allSites for _ in 1:legs]...)
                    if intersect(left, occupied) |> isempty && intersect(left, vacant) |> isempty
                        continue
                    end
                    operator, left, factorLeft = def(left, spins)
                    if factorLeft == 0
                        continue
                    end
                    ...
                    if factor ≠ 0
                        if issetequal(left, [1, 2, 4, 5])
                            println(left)
                            println(right)
                            println(combinedOperator)
                            println("----")
                        end
